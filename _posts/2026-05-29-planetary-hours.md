...
行星時刻（Planetary Hours）是源自希臘化時期占星傳統的時間劃分系統。它將每日從日出到日落、從日落到次日日出各分為十二等份，依迦勒底秩序（Chaldean Order）將七顆古典行星分配給每一時刻，用於擇時、儀式與占星實踐。

由於白晝與黑夜長度隨季節伸縮，每個「行星時」並非固定六十分鐘，必須依當地實際的日出日落時間計算。本工具會根據你選擇的城市與日期，計算當日完整的二十四個行星時，並顯示該日的月相、月齡與下一次滿月、新月的日期。

天文計算採用 SunCalc 函式庫（由 Vladimir Agafonkin 開發，BSD 授權），於瀏覽器端離線運算，不向任何外部伺服器傳送資料。城市搜尋使用 Open-Meteo 公開地理編碼 API。
